	...
	$.fn.fileUploader = function(config){
			...
			if (itr > 1){
				...
				$("#pxupload_form form").each(function(){
					...
					if (input_val != ""){
						...
						//	up_output = $(this).contents().ifind("#output").text();
							response = $(this).contents().text();
							var stat = response.substring(0,response.indexOf('|'));
							if (stat == "success"){
		...
		var px = {
			...
			appendForm: function(){
				...
				'<input type="hidden" name="app_id" id="app_id" value="'+ config.app_id + '"/>' +
				'</form>' + 
